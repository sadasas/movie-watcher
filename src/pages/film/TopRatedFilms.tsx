...
function TopRatedFilm({ scrollPosition }: { scrollPosition: ScrollPosition }) {
  ...
  const getDataMovieHandler = async (
    ...
    const { validData, hasNextItems } = await getTopRatedFilms(index, length);
...
export default trackWindowScroll(TopRatedFilm);
